...
function TextGet(TextTag) {
	let Result = TextScreenCache ? TextScreenCache.get(TextTag) : "";
	if (Character.length > 0) Result = Result.replace("PlayerName", Character[0].DisplayName);
	return Result;
}
...
function TextLoad(TextGroup) {
	...
	if ((TextGroup == null) || (TextGroup == "")) TextGroup = CommonScreen;
	const FullPath = "Screen/" + CommonModule + "/" + TextGroup + "/Text_" + TextGroup + ".csv";
...
function TextPrefetch(Module, TextGroup) {
	const file = "Screen/" + Module + "/" + TextGroup + "/Text_" + TextGroup + ".csv";
